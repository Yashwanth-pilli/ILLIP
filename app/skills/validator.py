...
_BLOCKED_PATTERNS = [
    ...
    # Only block exec() when it's NOT inside a string/comment
    # We use AST for this instead — see _ast_scan
...
_WARN_PATTERNS = [
    ...
    (r"ANTHROPIC_API_KEY|OPENAI_API_KEY", "hardcoded API key reference"),
]

# AST node types that are hard-blocked
_BLOCKED_AST_CALLS = {
...
def _ast_scan(code: str, source_name: str) -> tuple[bool, str, list[str]]:
    ...
    for node in ast.walk(tree):
        ...
        # Warn on os.system / os.popen attribute access
        if isinstance(node, ast.Attribute):
            if node.attr in ("system", "popen", "execv", "execve", "fork"):
                warnings.append(f"Suspicious os.{node.attr}() call detected")
